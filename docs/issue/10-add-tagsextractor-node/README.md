...
| `test_a_phrase_the_vocabulary_cannot_spell_comes_from_the_wiki` | "a man is taking off her bra", `min_count=500` | `1boy`, `solo`, `undressing`, `bra`이고 `taking off → undressing` 행의 `via`가 `wiki`다 |
| `test_solo_sends_the_wiki_to_the_next_tag_down` | 정의에 "someone else"가 든 태그 중 1위가 `another` 태그일 때, `solo`로 조회 | `another`가 없는 다음 태그가 나온다 |
| `test_solo_sends_a_spelling_to_the_next_tag_down` | "a girl undressing another" | `1girl`, `solo`, `undressing`. `undressing another` 대신 다음 철자 `undressing`이다 |
| `test_subject_off_still_rules_and_leaves_the_table` | 같은 문장, `subject=False` | 출력은 `undressing`뿐이고 `(count)` 행 두 개의 판정이 `subject off`다 |
